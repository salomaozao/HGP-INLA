...
"inla.rgeneric.blockNNGP.model" <- function(
  ...
  interpret.theta <- function() {
    return(
      list(
        ...
        phi = b.phi - (b.phi-a.phi) / (1 + exp(theta[2L])),
        nu = 1 / (1 + exp(-theta[3L]))
      ) # b - (b-a)/(1+exp(theta2))
    )
  }
  ...
  Q <- function() {
    ...
    R <- exp((-1) * (coords.D / param$phi)^param$nu)
    diag(R) <- 1
    C <- param$sigmasq * R
  ...
  log.prior <- function() {
    ...
    res <- dgamma(param$sigmasq, 1, 5e-05, log = TRUE) + log(param$sigmasq) +
      -2*(log(b.phi-a.phi) - log(b.phi-param$phi)) + log((b.phi-a.phi)/(b.phi-param$phi)-1) +
      2*log(param$nu) - (log(param$nu)-log(1-param$nu)) + dbeta(param$nu,a.nu,b.nu,log=TRUE)
    return(res)
  }
  ...
  initial <- function() {
    return(c(0, 0, log((a.nu-1)/(b.nu-1))))
  }
